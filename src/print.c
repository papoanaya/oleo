 ...
 *
 * $Id: print.c,v 1.11 2012/09/03 01:50:27 papo Exp $
 *
...
void
csvprint_region_cmd(struct rng *print, FILE *fp) {
  Global->CurrentPrintDriver=&CsvPrintDriver;
  ...
  int	print_width, print_height, totht, totwid,
    ht, npages, current_size,
    xpoints, xspaces, xpointsafter;
  char	pg[32];
  char	*title = NULL;
  /* Font heights */
  int	*heights = NULL, i,j,k;

  /* Start Printing */
  Global->CurrentPrintDriver->job_header(title, 0, fp);
  Global->CurrentPrintDriver->paper_size(print_width, print_height, fp);
  Global->CurrentPrintDriver->set_border(0.0, 0.0, fp);
  ...
  for (c_lo = print->lc, c_hi = 0; c_hi != print->hc; c_lo = c_hi + 1) {
    ...
    c_hi = print->hc;		/* The last column to print on the current page */
    ...
    for (rr = print->lr; rr <= print->hr; rr++) {
      ...
      for (cc = c_lo; cc <= c_hi; cc++) {
...

        {
          ...
          if (cc==c_hi) {
            ...
            Global->CurrentPrintDriver->field(ptr, w, 0,
                                              1, xspaces, fp);
          }
      ...
      Global->CurrentPrintDriver->newline(1, fp);
...
void
grprint_region_cmd(struct rng *print, FILE *fp) {
  Global->CurrentPrintDriver=&GroffPrintDriver;
  ...
  memset(justifications,'\0',30000);

  k=0;
  for (rr = print->lr, i=0; rr <= print->hr; rr++, i++) {
    /* Find largest font on the row */
    current_size = 0;
    for (cc = print->lc, j=0; cc <= print->hc; cc++,j++) {
      cp = find_cell(rr, cc);
    ...
    heights[i] += get_height(rr);  /* times font size*/
    justifications[k]='\n';
    k++;
  ...
  for (cc = print->lc; cc <= print->hc; cc++) {
    totwid += get_width(cc);  /*times font width*/
  }
  ...
  for (c_lo = print->lc, c_hi = 0; c_hi != print->hc; c_lo = c_hi + 1) {
    ...
    /* width must be multiplied times font width */
    for (w = get_width(cc);
    ...
    for (rr = print->lr; rr <= print->hr; rr++) {
      ...
      if (totht == 0) {
        ...
        Global->CurrentPrintDriver->page_header(justifications, fp);
        npages++;
      }
      ...
      for (cc = c_lo; cc <= c_hi; cc++) {
        ...
        w = w * ((current_size > 0) ? current_size : 1);
